...
-- Run ONCE on Railway PostgreSQL (Query tab)
...
CREATE INDEX IF NOT EXISTS idx_submissions_level   ON submissions(current_level);

-- ─────────────────────────────────────────────────────────────
-- Admin seeded automatically by server/index.js on startup.
-- Username and password come from ADMIN_USERNAME / ADMIN_PASSWORD
-- environment variables in Railway.
--
-- To manually reset the admin password via SQL run:
--   UPDATE moderators
--   SET password_hash = crypt('YourNewPassword', gen_salt('bf',12))
--   WHERE username = 'academitrack_admin';
-- ─────────────────────────────────────────────────────────────
